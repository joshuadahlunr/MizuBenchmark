...
const uint64_t number_size = sizeof(numbers)/sizeof(numbers[0]);
...
int main() {
	...
	while(changed) {
		changed = false;
		for(uint64_t i = 1; i < number_size; ++i)
	...
	for(uint64_t i = 0; i < number_size; ++i) {
		volatile int first = numbers[i];
		assert(first == sorted[i]);
	}
}